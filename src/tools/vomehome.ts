...
export function registerVomeHomeTools(server: McpServer, ctx: ToolContext): void {
		...
		async () =>
			runTool(ctx.logger, "vomehome_list_instances", async () => {
				...
				return jsonResult({
					...
					instances: instances.map(serialiseInstance)
				});
			})
	);
	...
	server.registerTool(
		"vomehome_create_instance",
		{
			...
			description:
				"Create a new Home Assistant instance on VomeHome — useful for spinning up a throwaway test install. This is a heavyweight action: it requires both HA_ALLOW_WRITE=true and VOMEHOME_ALLOW_CREATE=true, and may be subject to your account's instance limit and billing.",
			inputSchema: {
